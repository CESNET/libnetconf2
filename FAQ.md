...
__A:__ This is the standard TCP behavior. The client will not immediately detect that the connection
   was terminated unless it tries to send some data or unless a specific timeout occurs.

   In particular, the timeout you're encountering is most likely affected by the `tcp_fin_timeout` kernel parameter,
